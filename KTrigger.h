...
enum {
	...
	ROTATE_GROUP_END_ID,
	SINGLE_SCALE_GROUP_ID,
	SINGLE_SCALE_DISK_ID,
	SINGLE_SCALE_GROUP_END_ID,
	MAX_DUR_DISK_ID,
	XFER_MODE_DISK_ID,
...
typedef struct CheckLayer {
	PF_ParamType 	LayerSource;
	A_long 	 		storageId;
	PF_FpLong		keyTime;
} CheckLayer; // 需要签出的图层包

typedef struct CombinedLayerInfo {
	PF_ParamType 	LayerSource;
	PF_FpLong		keyTime;
	PF_FloatMatrix 	matrix;
} CombinedLayerInfo;

typedef struct CombinedLayerPack {
	A_long     xfer;
	PF_Rect    MaxRect;
	std::vector<CombinedLayerInfo> pack;
} CombinedLayerPack;
